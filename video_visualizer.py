...
class VideoVisualizer:
    ...
    def draw_panoptic_seg_predictions(
        self, frame, panoptic_seg, segments_info, area_threshold=None, alpha=0.5, add_frame = True
    ):
        frame_visualizer = Visualizer(frame, self.metadata)
        ...
        for mask, sinfo in pred.semantic_masks():
            ...
            frame_visualizer.draw_binary_mask(
                mask,
                color=mask_color,
                text=self.metadata.stuff_classes[category_idx] if add_frame else None,
                alpha=alpha,
